...
const BidPage = () => {
  ...
  const [timeLeft, setTimeLeft] = useState(
    calculateTimeLeft(
      location.state.cropDetails.startdate,
      location.state.cropDetails.duration
    )
  );
            ...
            <Paper>
              <div style={{ padding: "10px" }}>
                ...
                <Typography component="p" sx={{ margin: "5px 0" }}>
                  Sold By:{" "}
                  <b>{`${location.state?.cropDetails.owner?.firstname} ${location.state?.cropDetails.owner?.lastname}`}</b>
                </Typography>
            ...
            <Paper>
              ...
              <List disablePadding spacing={2}>
                {data.map((user) => (
                  <>
                    <ListItem
                      key={`${user.username}${user.bidprice}${user.time}`}
                    >
                      <ListItemButton sx={{ justifyContent: "space-between" }}>
                        <ListItemText
                          primary={`By "${user.username}"`}
                          secondary={`Rs.${user.bidprice} @ ${new Date(
                            user.time * 1000
