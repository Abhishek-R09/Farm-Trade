...
cors = CORS(app, resources={
            r"/getTopFive": {"origins": "http://localhost:3000"},
            r"/getBottomFive": {"origins": "http://localhost:3000"},
            r"/getCommodityDetails/*": {"origins": "http://localhost:3000"}})
...
annual_rainfall = [29, 21, 37.5, 30.7, 52.6,
                   150, 299, 251.7, 179.2, 70.5, 39.8, 10.9]
base = {
    "Paddy": 1245.5,
    "Arhar": 3200,
    "Bajra": 1175,
...
class Commodity:
    ...
    def __init__(self, csv_name):
        ...
        # print("The model training score is" , self.regressor.score(self.X, self.Y))
        # print(cross_val_score(self.regressor, self.X, self.Y))
...
@app.route('/getBottomFive', methods=["GET"])
def bottomFive():
    bottom5 = TopFiveLosers()
    return jsonify({"bottom5": bottom5})


@app.route('/getCommodityDetails/<commodityName>')
def commodityDetails(commodityName):
    max_crop, min_crop, forecast_crop_values = TwelveMonthsForecast(
        commodityName)
    prev_crop_values = TwelveMonthPrevious(commodityName)
    ...
    current_price = CurrentMonth(commodityName)

    crop_data = crops.crop(commodityName)
    context = {
        "name": crop_data[4],
        "max_crop": max_crop,
        "min_crop": min_crop,
...
def TopFiveWinners():
    ...
    for i in commodity_list:
        current_predict = i.getPredictedValue(
            [float(current_month), current_year, current_rainfall])
        current_month_prediction.append(current_predict)
        prev_predict = i.getPredictedValue(
            [float(prev_month), current_year, prev_rainfall])
        prev_month_prediction.append(prev_predict)
        change.append((((current_predict - prev_predict) * 100 /
                      prev_predict), commodity_list.index(i)))
...
def CurrentMonth(name):
    ...
    for i in commodity_list:
        if name == str(i.getCropName().split("/")[1].lower()):
            commodity = i
            break
...
def TwelveMonthsForecast(name):
    ...
    for i in range(1, 13):
        if current_month + i <= 12:
            month_with_year.append(
                (current_month + i, current_year, annual_rainfall[current_month + i - 1]))
        else:
            month_with_year.append(
    ...
    for i in range(0, len(wpis)):
        ...
        x = x.strftime("%b %y")
        crop_price.append(
            [x, round((wpis[i] * base[name.capitalize()]) / 100, 2), round(change[i], 2)])
   # print("forecasr", wpis)
...
def TwelveMonthPrevious(name):
    ...
    crop_price = []
    for i in commodity_list:
        print(i.getCropName().split("/")[1].lower())
        if name == str(i.getCropName().split("/")[1].lower()):
            commodity = i
            break
    month_with_year = []
    ...
    for i in range(0, len(wpis)):
        ...
        x = x.strftime("%b %y")
        crop_price.append(
            [x, round((wpis[i] * base[name.capitalize()]) / 100, 2)])
   # print("previous ", wpis)
    new_crop_price = []
